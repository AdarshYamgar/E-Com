...
function AppState(props){
     ...
     const [userOrderDtl,setUserOrderDtl]=useState([])
  ...
  transition: Bounce,
      ...
      return true;
       ...
       console.error("Error fetching employees:",error.response?.message || error.message);
       console.log("status code",error.response?.status);
       if(error.response?.status === 909 || error.response?.status === 999){
...
const User_Order=async(req,res)=>{
    const user=JSON.parse(localStorage.getItem("User"))
    if(!user){
      console.log("No Valid token found,redirecting to the login ...")
      return;
    }
    try{
      const res=await axios.get("http://localhost:8080/api/payment/userOrder",{
        headers:{
          "Content-Type":"Application/json",
          "Auth":user
        },
      ...
      console.log("User order is",res.data);
      setUserOrderDtl(res.data)
