...
const Checkout=()=>{
  const navigate=useNavigate();
    const {cart,userAddress,user,ClearCart,profile}=useContext(AppContext)
    const [qty,setQty]=useState(0);
    const [price,setPrice]=useState(0);
   // console.log("cartttt",cart)
   console.log(userAddress)
   console.log("User details in checkout top is",profile)
   useEffect(()=>{
    let qty=0;
    let price=0;
    if(cart?.items){
...
const handlePayment=async()=>{
  ...
  "modal": async function (response){
    console.log("I am in payment cancel block")
    const paymentData={
      orderId:response.razorpay_order_id,
      paymentId:response.razorpay_payment_id,
      signature:response.razorpay_signature,
    ...
    return(
        ...
        <div className="container my-3">
          <div className="table-responsive">
            ...
            <h1 className="text-center m-4 fs-2 fw-bold">Order Summery</h1>
    ...
    <tr>
      ...
      <td>
        ...
        <div>
          ...
          <button onClick={handlePayment} className=" btn btn-info rounded-3 ">Proceed To Pay</button>
           </div>
        </div>
        )}
